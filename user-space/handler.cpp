...
int net_protect_handler(struct nl_cache_ops *unused, struct genl_cmd *genl_cmd, struct genl_info *genl_info, void *arg) {
    ...
    case NET_PROTECT_REPORT: {
        net_port_t port = nla_get_u16(genl_info->attrs[HACKERNEL_A_PORT]);
        net_perm_t perm = nla_get_s32(genl_info->attrs[HACKERNEL_A_PERM]);

        LOG("net: port=[%d] perm=[%d]", port, perm);
        break;
    }
    default: {
